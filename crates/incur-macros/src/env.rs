//! `#[derive(IncurEnv)]` — generates `IncurSchema` for environment-variable bindings.
...
use crate::common::{
    default_value_tokens, doc_description, field_type_tokens, is_option_type, parse_incur_attrs,
    snake_to_kebab,
};
...
pub fn derive(input: TokenStream) -> TokenStream {
    ...
    let fields = match &input.data {
        Data::Struct(data) => match &data.fields {
            ...
            _ => {
                return syn::Error::new_spanned(
                    &input.ident,
                    "IncurEnv can only be derived for structs with named fields",
                )
                .to_compile_error()
                .into();
            }
        ...
        _ => {
            return syn::Error::new_spanned(
                &input.ident,
                "IncurEnv can only be derived for structs",
            )
            .to_compile_error()
            .into();
        }
    ...
    let field_meta_entries: Vec<TokenStream2> = fields
        ...
        .map(|f| {
            ...
            let env_name_tokens = match &attrs.env_name {
                Some(env) => quote! { Some(#env) },
                None => {
                    // Fall back to SCREAMING_SNAKE_CASE of the field name.
                    let fallback = field_name_str.to_uppercase();
                    quote! { Some(#fallback) }
                }
            };
            ...
            fn from_raw(
                ...
                serde_json::from_value::<Self>(obj).map_err(|e| {
                    ::incur::errors::ValidationError {
                        message: format!("Failed to parse env: {}", e),
