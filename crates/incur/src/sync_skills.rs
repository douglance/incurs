...
pub async fn sync(
    ...
    let tmp_dir = std::env::temp_dir().join(format!("incur-skills-{}-{}", name, std::process::id()));
...
fn meta_path(name: &str) -> PathBuf {
    ...
    data_home.join("incur").join(format!("{}.json", name))
}
...
mod tests {
    ...
    fn test_meta_path() {
        ...
        assert!(path.to_string_lossy().contains("incur"));
